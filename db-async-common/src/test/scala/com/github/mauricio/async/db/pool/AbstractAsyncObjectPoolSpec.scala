 ...
 */
abstract class AbstractAsyncObjectPoolSpec[T <: AsyncObjectPool[Widget]](implicit
  tag: TypeTag[T]
  ...
  s"the ${tag.tpe.erasure} variant of AsyncObjectPool" should {
    ...
    "queue requests after running out" in {
      ...
      Await
        .result(failedFuture, Duration.Inf) must throwA[PoolExhaustedException]
...
class SingleThreadedAsyncObjectPoolSpec
  ...
  import AbstractAsyncObjectPoolSpec._
  private val pools =
    collection.concurrent.TrieMap[SingleThreadedAsyncObjectPool[_], Boolean]()
