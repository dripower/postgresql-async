 ...
 */
private[db] object Execution {

  /**
   * Executes in the current thread. Uses a thread local trampoline to make sure the stack doesn't overflow. Since this
   * ExecutionContext executes on the current thread, it should only be used to run small bits of fast-running code. We
   * use it here to run the internal iteratee code.
   *
