...
class TimestampEncoderDecoder extends ColumnEncoderDecoder {
  ...
  protected val timezonedPrinter: DateTimeFormatter = new DateTimeFormatterBuilder()
    .appendPattern(BaseFormat)
    .appendFraction(ChronoField.NANO_OF_SECOND, FractionSize, FractionSize, true)
    .appendOffset("+HH:mm", "Z")
    .toFormatter()

  protected val nonTimezonedPrinter: DateTimeFormatter = new DateTimeFormatterBuilder()
    .appendPattern(BaseFormat)
    .appendFraction(ChronoField.NANO_OF_SECOND, FractionSize, FractionSize, true)
    .toFormatter()
  ...
  override def encode(value: Any): String = {
    value match {
      case t: Timestamp      => this.timezonedPrinter.format(t.toInstant.atZone(systemZone))
      case t: Date           => this.timezonedPrinter.format(t.toInstant.atZone(systemZone))
      case t: Calendar       => this.timezonedPrinter.format(t.toInstant.atZone(systemZone))
