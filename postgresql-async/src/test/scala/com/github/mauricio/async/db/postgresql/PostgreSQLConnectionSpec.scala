...
class PostgreSQLConnectionSpec extends Specification with DatabaseTestHelper {
  ...
  final val sampleArray =
    Array[Byte](83, 97, 121, 32, 72, 101, 108, 108, 111, 32, 116, 111, 32, 77, 121, 32, 76, 105, 116, 116, 108, 101, 32,
      70, 114, 105, 101, 110, 100)
  ...
  val preparedStatementInsert3 =
    " insert into prepared_statement_test (name) values ('Peter Parker')"
  ...
  "handler" should {
    ...
    "execute a prepared statement with parameters" in {
      ...
      withHandler { handler =>
        ...
        val queryResult =
          executePreparedStatement(handler, select, Array("Peter Parker"))
    ...
    "fail login using with an invalid credential exception" in {
      ...
      val configuration = new Configuration(
        username = "postgres_invalid_user",
      ...
      Try {
        ...
        case _: GenericDatabaseException => ok
      }
    }
    ...
    "send data to a bytea column" in {
      ...
      withHandler { handler =>
        ...
        executePreparedStatement(
          handler,
          insert,
          Array(ByteBuffer.wrap(sampleArray))
        )
        executePreparedStatement(
          handler,
          insert,
          Array(Unpooled.copiedBuffer(sampleArray))
        )
