  ...
  "connection" should {
    ...
    "close when prepared statement too much" in {
      ...
      withConnection { c =>
        ...
        val rs = (1 to 1000).map { i =>
          val rs = executePreparedStatement(
            c,
            s"select * from posts where id > ? and $i = $i",
            i
          ).rows.get
        }
        rs.size === 1000
    ...
    "bind timestamp parameters to a table" in {
      ...
      withConnection { connection =>
        ...
        row("created_at_timestamp") === timestamp.toLocalDateTime
    ...
    "support setting Some(value) to a column" in {
      withConnection { connection =>
        ...
        val moment = LocalDateTime
          .now()
          .withNano(0) // cut off fractional seconds to match timestamp
        executePreparedStatement(
          connection,
          "INSERT INTO timestamps (moment, id) VALUES (?, ?)",
          Some(moment),
