...
object TransactionSpec {
  val CreateTable =
    """CREATE TABLE IF NOT EXISTS transaction_test(id bigint primary key)"""
  val BrokenInsert =
    """INSERT INTO users (id, name) VALUES (1, 'Maurício Aragão')"""
    ...
    "runs commands for a transaction in a single connection" in {
      ...
      withPool { pool =>
        val operations = pool.inTransaction { connection =>
          connection
            .sendPreparedStatement(TransactionInsert, List(id))
            .flatMap { result =>
              connection
                .sendPreparedStatement(TransactionInsert, List(id))
                .map { failure =>
