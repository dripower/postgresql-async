    ...
    "be able to execute create stored procedure" in {
      withConnection { connection =>
        val future =
          for (
            drop <- connection
              .sendQuery("DROP PROCEDURE IF exists helloWorld;");
    ...
    "be able to call stored procedure" in {
      withConnection { connection =>
        val future =
          for (
            drop <- connection
              .sendQuery("DROP PROCEDURE IF exists constTest;");
            create <- connection.sendQuery(
              """
    ...
    "be able to call stored procedure with input parameter" in {
      withConnection { connection =>
        val future =
          for (
            drop <- connection
              .sendQuery("DROP PROCEDURE IF exists addTest;");
            create <- connection.sendQuery(
              """
    ...
    "be able to remove stored procedure" in {
      withConnection { connection =>
        val createResult: Option[ResultSet] = awaitFuture(
          for (
            drop <- connection
              .sendQuery("DROP PROCEDURE IF exists remTest;");
            create <- connection.sendQuery(
              """
